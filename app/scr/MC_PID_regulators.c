...
* 29/05/08 v2.0
********************************************************************************
*@brife 此文件主要是将PID的运算做成单独的运算单元，并对PID结构体进行初始化。
		1.PID运算包括最终输出限制赋幅值，可选D项，抗饱和积分等
*******************************************************************************/
...
void PID_Speed_Coefficients_update(s16 motor_speed, PID_Struct_t *PID_Struct)
{
	...
	if ( motor_speed < 0)  
	{
		motor_speed = (u16)(-motor_speed);   
	}
	//1Hz=60rpm 以下的速度PID
	if ( motor_speed <= Freq_Min ) 
	{
...
s16 PID_Regulator(s16 hReference, s16 hPresentFeedback, PID_Struct_t *PID_Struct)
{
	...
	houtput_32 = (wProportional_Term/PID_Struct->hKp_Divisor+ PID_Struct->wIntegral/PID_Struct->hKi_Divisor + wDifferential_Term/PID_Struct->hKd_Divisor); 
	#else  
