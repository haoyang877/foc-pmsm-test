  ...
  *          Liquid Crystal Display Module of STM3210C-EVAL board.（液晶显示器模块）
  ******************************************************************************
  *@brife 此文件是STM32的LCD模块的驱动文件，可以当做为硬件库 驱动LCD ILI9320模块
		1.修改时应注意相关的引脚设置
  *
...
void LCD_WriteReg(u8 LCD_Reg, u16 LCD_RegValue)
{
 ...
 /* SPI_I2S_SendData(LCD_SPI, LCD_RegValue>>8);
  ...
  GPIO_SetBits(LCD_RS_PORT,LCD_RS_PIN);   
  GPIO_Write(LCD_PORT_PORT,LCD_RegValue>>8);
  //Delay(5);		
  GPIO_ResetBits(LCD_WR_PORT,LCD_WR_PIN); 
 // Delay(10);
  GPIO_SetBits(LCD_WR_PORT,LCD_WR_PIN);
  GPIO_Write(LCD_PORT_PORT,LCD_RegValue);
  //Delay(10);		
  GPIO_ResetBits(LCD_WR_PORT,LCD_WR_PIN); 
 // Delay(10);
  GPIO_SetBits(LCD_WR_PORT,LCD_WR_PIN);
...
void LCD_CtrlLinesConfig(void)
{
  GPIO_InitTypeDef GPIO_InitStructure;
   unsigned int i;
  /* Enable GPIO clock */
  RCC_APB2PeriphClockCmd(LCD_NCS_GPIO_CLK|LCD_RST_GPIO_CLK, ENABLE);
...
void LCD_GPIOConfig(void)
{
  ...
  /* Enable GPIO clock */
  RCC_APB2PeriphClockCmd(LCD_WR_CLK|LCD_PORT_CLK|LCD_RD_CLK|LCD_RS_CLK, ENABLE);
  ...
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
  GPIO_Init(LCD_PORT_PORT, &GPIO_InitStructure);   
